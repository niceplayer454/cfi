...
{
    public class RcMailGroupDa
    {
        public static List<RcMailGroup> GetAll()
        {
            Database db = DatabaseFactory.CreateDatabase();

            DbCommand dbCommand = db.GetSqlStringCommand(String.Format(@"SELECT [Code] ,[Title] ,[BU] ,[MailList] ,[Sort] ,[Optor] ,[OpTime] ,[Deleted] 
FROM {0}", DataDictionaryEntryDa.GetDictionaryTableName(DictionaryName.RcMailGroup)));

            List<RcMailGroup> entries = new List<RcMailGroup>();
...
WHERE [Code] = N'{0}'",
        ...
        private static RcMailGroup PopulateEntry(IDataReader dataReader)
        {
